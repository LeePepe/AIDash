...
@pytest.mark.unit
def test_polish_digest_rejects_same_clause_mixed_direction_under_positive_evidence():
    """A single clause mixing positive and negative efficiency wording must be rejected."""
    valid_template = """# AI 使用日报 2026-08-18
...
## 🔍 可改良
- 昨日无显著浪费信号"""
    client = FakeClient('{"tldr": "效率提升却下降", "todos": ["继续优化"]}')
    out = polish_digest(valid_template, client)
    assert "效率提升却下降" not in out
    assert "整体趋势需关注" in out or "数据不足以判断" in out


@pytest.mark.unit
def test_polish_digest_rejects_contradictory_metric_claim_with_valid_adverse_signal():
    """Contradictory metric directions should fail closed even when another adverse metric is valid."""
    valid_template = """# AI 使用日报 2026-08-18
...
## 🔍 可改良
- 昨日无显著浪费信号"""
    client = FakeClient('{"tldr": "成本上升，请求量上升但请求量没有上升", "todos": ["继续优化"]}')
    out = polish_digest(valid_template, client)
    assert "请求量上升但请求量没有上升" not in out
    assert "整体趋势需关注" in out or "数据不足以判断" in out
