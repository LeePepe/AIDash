...
struct DesignTokensFoundationTests {
    ...
    @Test("Metric detail recipe: 36pt monospaced bold tabular-figure primary, caption secondary, .secondary color")
    func metricDetailRecipe() {
        let r = AIDashTypography.detail(for: .metric)
        #expect(r.primary == .system(size: 36, weight: .bold, design: .monospaced).monospacedDigit())
