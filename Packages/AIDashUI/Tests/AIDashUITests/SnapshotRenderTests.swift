...
struct SnapshotRenderTests {
    ...
    func renderSample() {
        ...
        let overview = container("Overview", .grid, [
            card(.metric, .small, .success, #"{"items":[{"label":"PRs merged","value":12,"trend":"up","higherIsBetter":true,"series":[4,6,5,8,7,10,9,12]}]}"#),
            card(.metric, .small, .neutral, #"{"items":[{"label":"Build time","value":124,"unit":"s","trend":"down","higherIsBetter":false,"series":[180,170,165,150,148,140,132,124]}]}"#),
            card(.metric, .small, .accent, #"{"items":[{"label":"Coverage","value":87,"unit":"%","ratio":0.87}]}"#),
            card(.metric, .small, .warning, #"{"items":[{"label":"Open incidents","value":3,"trend":"up","higherIsBetter":false,"series":[0,1,1,2,1,2,3,3]}]}"#),
        ])

        let today = container("Today", .hero, [
            card(.digest, .hero, .neutral, #"{"title":"A strong, incident-light day","body":"Twelve PRs merged across Sapphire and Basalt, with the v9-blocking crash finally resolved. The design-system migration crossed 70%. Build times are trending down.","sections":[{"heading":"Shipped","paragraphs":["SAP-301 crash fix (unblocks v9).","Cache rework cut CI 30%."]},{"heading":"Blocking today","paragraphs":["Perf review feedback due 5pm."]}]}"#),
            card(.insight, .wide, .accent, #"{"title":"Build-cache rework is paying off","body":"Median CI dropped 180s to 124s over the week — the single biggest developer-time win this sprint."}"#),
            card(.agentSummary, .wide, .success, #"{"agentName":"Multica","completed":[{"title":"Merged 3 Sapphire PRs","ref":"https://github.com/x/1"},{"title":"Regenerated changelog"}],"stats":[{"label":"PRs","value":3},{"label":"Reviews","value":9},{"label":"Hours","value":6.5}]}"#),
