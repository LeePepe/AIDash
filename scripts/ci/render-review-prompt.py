...
渲染结果写 stdout。SECURITY_NOTICE / EVIDENCE_RULES 必须非空，其余字段允许空串。
...
PLACEHOLDERS = ("CHANGED", "TRUNCATED", "DIFF", "SECURITY_NOTICE", "EVIDENCE_RULES", "SCOPE_EVIDENCE")
...
def main() -> int:
    ...
    for name in ("SECURITY_NOTICE", "EVIDENCE_RULES"):
        if not values[name].strip():
            print(f"render-review-prompt: missing trusted section: {name}", file=sys.stderr)
            return 4
