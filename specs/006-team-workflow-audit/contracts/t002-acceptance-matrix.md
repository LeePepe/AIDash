...
| D10 | T022/T025 | Valid raw sidecar bytes | `ArtifactSidecarWire` schema version 1, matching snapshot ID, stable sidecar ID, raw artifact entries, optional grill strings, and exact full-file byte SHA-256 survive enrichment without a circular self-declared sidecar hash. |
| D11 | T022/T025 | Missing generic/team relationship or P0/P1 chain, unsafe mandatory URL, bad hash/parent, or mandatory wire entry at size boundary | Canonical raw-entry bytes use sorted keys, compact separators, unescaped Unicode, and exclude envelope/derived fields; 65,536 bytes accepts, 65,537 rejects before raw append. |
...
| D16 | T022/T025 | Raw artifact entry includes `sidecarID`, `sidecarSHA256`, `encodedByteCount`, `urlStatus`, or another derived key | Unknown-wire-field rejection; a valid wire entry receives those derived values only after exact sidecar hashing and canonical raw-entry counting. |
| D17 | T022/T025 | Raw entry omits a nullable key, duplicates an object member, or an unchanged entry appears under sidecar-only byte/grill changes | Omitted nullable key and duplicate member reject before canonicalization; explicit JSON null is canonical; unchanged wire bytes keep the same artifact `entitySHA256` while the independently hashed sidecar may collide. |
