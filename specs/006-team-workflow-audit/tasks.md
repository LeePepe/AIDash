...
| Functional acceptance | Fast success and ordinary failure return their real status; deadline returns 124; `bash -e` callers emit the fail-closed diagnostic; TERM→KILL removes `env → bash → child` descendants; leader-exits-zero with lingering descendants cleans promptly and returns 0; TERM-trapping leaders cannot hide a timeout; no PID or stdout/stderr pipe leaks; behavior is portable on GitHub Linux and the macOS review runner; existing sticky-comment/security/900-second semantics are unchanged |
| Exact verification | Normal `git commit` and `git push` with configured hooks; hook-selected RepoInfra syntax plus `/usr/bin/python3 -m pytest scripts/ci/tests scripts/context/tests -q` must exit 0; CI `review-gate (pytest)` must pass |
| Dependencies / slice | None; first recovery gate and hard prerequisite for T021; do not transplant failed PR #202 patch |
...
RepoInfra hook suite fails its three watchdog regressions before T021 can
publish normally. T019 is the merge-first expand step that keeps T005
AIDashCore-only and repository-buildable.
...
- T020 uses its own issue, branch, workspace, PR, and RepoInfra evidence; PR
