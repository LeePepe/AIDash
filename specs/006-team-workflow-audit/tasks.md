...
| Interface / contract | Named read-only bundles expose accepted snapshot/sidecar provenance, cohort/cursors, axes/effectiveness, collision summary, and required entity/count inputs for mandatory P0/P1 findings plus generic/team/P0/P1 artifacts; L4 has no published/omitted/externalized result |
| Functional acceptance | Query grains are explicit; latest ordering is deterministic; finding subject/responsibility and parented collisions survive; sidecar ID/hash reaches every required input; required counts derive from immutable facts only; columns named `published*`, `omitted*`, or `externalized*` are absent; empty warehouse returns an empty/degraded bundle |
...
| Interface / contract | `contracts/card-payload.md` and `data-model.md`: one `teamAudit` type, eight variants, explicit finding identity, parented collisions, snapshot+sidecar provenance, L5-computed coverage, full-report/externalization types, locked enums/invariants, and no content-derived size downgrade |
| Functional acceptance | Round trips cover exact source fields; findings require subject/responsibility; collisions require parent snapshot ID/hash; every payload requires sidecar ID/hash; baseline/cursor, parts, axes/effectiveness, states, repeat union, L5 coverage reconciliation, untrusted URLs, and encoded size are tested; 262,144 accepts/262,145 mandatory rejects; CardType count 10→11; unknown enums use fallback |
...
| Interface / contract | Fetch T004 immutable required inputs; pack the US1 overview plus every mandatory P0/P1 finding and generic/team/P0/P1 artifact; compute final `PublicationCoverage`/full-report state in L5 after packing; emit snapshot+sidecar provenance and deterministic IDs |
| Functional acceptance | US1 publishes overview and all mandatory findings/artifacts independently; L5—not L4—computes published/omitted/externalized counts after final packing and required/published pairs match; mandatory invalid URLs/oversize reject; optional invalid links are not part of mandatory counts; finding identity, collision parent, sidecar ID/hash, axes/limitations survive; payloads ≤262,144; golden freezes seams; default digest invokes no audit |
