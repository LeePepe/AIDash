...
```json
{"items":[
  {"label":"PRs merged","value":12,"unit":"","trend":"up","higherIsBetter":true,"context":"Atlas · this week","series":[4,6,5,8,7,10,9,12]},
  {"label":"Coverage","value":87,"unit":"%","ratio":0.87,"context":"Atlas"}
]}
```
| 字段 | 类型 | 说明 |
|---|---|---|
...
| **`context`** | string? | **上下文小字**:这个数字是哪个项目/什么时间范围(如 `Atlas · 本周`)。渲染在标签下方。**强烈建议填**,否则用户不知道数字指什么 |
